...
- Zero or more `stage=value` pairs. Stage keys: `ux, conf, wire, assets, ui, back, cms, ok`.
...
1. Edit the row's stage emoji(s) — keep all 13 columns intact.
...
`Back 🟢` if the data path is wired & verified live — not merely coded. `CMS` tracks the
content *in* the plumbing (⚪ = page has no CMS content; 🟠 = team must check/complete —
it doubles as the client content chase-list). If a claimed stage isn't actually
verifiable yet, say so and leave it.
