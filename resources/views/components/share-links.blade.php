...
@php
    ...
    $shareMessage = $message ?? __('components.share.message', ['title' => $title, 'date' => $date, 'url' => $url]);
...
<div {{ $attributes->class('share-band__channels') }} x-data="{ copied: false }">
    ...
    <button type="button"
        ...
        :aria-label="copied ? @js(__('components.share.copied_aria')) : @js(__('components.share.copy_aria'))">
        <flux:icon.link class="share-band__copy-icon" aria-hidden="true" />
        <span x-show="!copied">{{ __('components.share.copy') }}</span>
        <span x-show="copied" x-cloak aria-live="polite">{{ __('components.share.copied') }}</span>
    </button>

    {{-- WhatsApp --}}
    <a href="{{ $whatsappUrl }}" target="_blank" rel="noopener"
        class="share-band__icon share-band__icon--whatsapp"
        aria-label="{{ __('components.share.whatsapp') }}">
    ...
    <a href="{{ $facebookUrl }}" target="_blank" rel="noopener"
        ...
        aria-label="{{ __('components.share.facebook') }}">
        <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
    ...
    <a href="{{ $mailtoUrl }}"
        ...
        aria-label="{{ __('components.share.email') }}">
